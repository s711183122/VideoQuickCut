...
class QuickCut:
    def __init__(self, tmp, time='', fps=30, type='n') -> None:
        ...
        self.time = int(time)
        self.type = type
        self.name = f'{self.type}_quickcut_{ttiimmee.strftime("%m_%d_%Y_%H_%M_%S", ttiimmee.localtime())}_({self.x0}_{self.y0})_({self.x1}_{self.y1})'
        output_path_type = f'{output_path}{self.type}/'
        if not os.path.isdir(output_path_type): os.mkdir(output_path_type)
        self.savename = f'{output_path_type}{self.name}.mp4'
...
def main():
    ...
    while cap.isOpened():
        ...
        if key == 27 or key == ord('q'):
            ...
            print(f'New QuickCut {key-48}')
            data['new_QuickCut'] = 1
            while True:
                key2 = cv2.waitKey()
                if key2 == 13 or key2 == key:
                    if not data['tmp']['L']:
                        ...
                        qc = QuickCut(data['tmp'], time=cap.get(1), fps=fps, type=f'{key}')
